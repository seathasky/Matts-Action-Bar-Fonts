...
do
    ...
    local function GetCurrentClassInfo()
        if cachedClassToken and cachedClassName then
            return cachedClassName, cachedClassToken
        end
        if UnitClass then
            cachedClassName, cachedClassToken = UnitClass("player")
        end
        return cachedClassName, cachedClassToken
    end
    ...
    local function IsTrackedPetClass()
        local _, classToken = GetCurrentClassInfo()
    ...
    local function GetCurrentClassLabel()
        local className = select(1, GetCurrentClassInfo())
    ...
    function MABF:SetupPetPassiveReminder()
        ...
        if self._petPassiveReminderTicker and self._petPassiveReminderTicker.Cancel then
            self._petPassiveReminderTicker:Cancel()
            self._petPassiveReminderTicker = nil
        end
        ...
        if C_Timer and C_Timer.NewTicker then
            self._petPassiveReminderTicker = C_Timer.NewTicker(0.5, function()
