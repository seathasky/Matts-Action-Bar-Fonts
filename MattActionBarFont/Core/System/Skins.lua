...
-- Uses a ticker instead of per-frame OnUpdate.
-----------------------------------------------------------
local globalSkinUpdater = CreateFrame("Frame")
local globalSkinTicker = nil

local function IsPlayerInCombatForSkinUpdates()
    return (InCombatLockdown and InCombatLockdown()) or (UnitAffectingCombat and UnitAffectingCombat("player")) or false
end

local function GetSkinUpdateInterval()
    if IsPlayerInCombatForSkinUpdates() then
        return 0.12
    end
    return 0.4
end

local function StopGlobalSkinTicker()
    if globalSkinTicker and globalSkinTicker.Cancel then
        globalSkinTicker:Cancel()
        globalSkinTicker = nil
    end
end

local function UpdateAllSkinnedButtons()
    for _, button in ipairs(skinnedButtons) do
        UpdateActionButtonState(button)
    end
end

local function StartGlobalSkinTicker()
    StopGlobalSkinTicker()
    if not (C_Timer and C_Timer.NewTicker) then
        return
    end
    globalSkinTicker = C_Timer.NewTicker(GetSkinUpdateInterval(), function()
        if not (globalSkinUpdater and globalSkinUpdater:IsShown() and IsMinimalThemeActive()) then
            return
        end
        UpdateAllSkinnedButtons()
    end)
end
...
globalSkinUpdater:RegisterEvent("PLAYER_REGEN_DISABLED")
globalSkinUpdater:RegisterEvent("PLAYER_REGEN_ENABLED")
globalSkinUpdater:SetScript("OnEvent", function()
    if globalSkinUpdater:IsShown() then
        StartGlobalSkinTicker()
    end
end)
globalSkinUpdater:Hide()
