...
local function InstallPetHotKeyColorLock(button)
    ...
    local function ForceBrightWhite(fs)
        ...
        local allowFade = fs._MABFBypassColorLock or (MABF and MABF.IsPetBarFadedOut and MABF:IsPetBarFadedOut())
        ...
        pcall(fs.SetAlpha, fs, allowFade and 0 or 1)
    ...
    local okAlpha = pcall(function()
        hooksecurefunc(hotKeyFont, "SetAlpha", function(fs, alpha)
            local allowFade = fs and (fs._MABFBypassColorLock or (MABF and MABF.IsPetBarFadedOut and MABF:IsPetBarFadedOut()))
            if fs and not fs._MABFColorLockApplying and tonumber(alpha) ~= 1 and not allowFade then
                ForceBrightWhite(fs)
            end
        end)
    end)
...
local function ApplyPetHotKeyVisualOverrides(button)
    ...
    if MABF and MABF.IsPetBarFadedOut and MABF:IsPetBarFadedOut() then
...
function MABF:UpdateMacroText()
    ...
    for _, button in ipairs(allButtons) do
        if button and button.Name then
            local macroHidden = MABF and MABF.ApplyMacroTextVisibility and MABF:ApplyMacroTextVisibility(button) or false
            if not macroHidden then
                if customFontsEnabled then
